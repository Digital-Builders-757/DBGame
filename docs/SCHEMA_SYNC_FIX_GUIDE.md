...
- âŒ "Binary files differ" when comparing types/database.ts
- âŒ Main couldn't fast-forward merge from develop
- âŒ Schema verification CI job was failing
...
{
  "devDependencies": {
    "supabase": "2.34.3"  // âœ… Pinned version
  },
  "scripts": {
    "types:regen": "... npx supabase@v2.34.3 ..."  // âœ… Uses pinned version
  }
}
```
...
- name: Login to Supabase (npx)
  run: npx -y supabase@v2.34.3 login --token "$SUPABASE_ACCESS_TOKEN"

- name: Generate types from remote schema (npx)
  run: npx -y supabase@v2.34.3 gen types typescript --project-id "$SUPABASE_PROJECT_ID" --schema public
```

âœ… **Both local and CI now use 2.34.3**
...
### âœ… Completed
- [x] Supabase CLI pinned to 2.34.3 in package.json
- [x] types:regen script uses pinned version
- [x] CI uses pinned version (npx -y supabase@v2.34.3)
...
### Before Fix:
```
Local (develop):  types generated with CLI v2.34.3 â†’ CRLF line endings
Main branch:      types from older generation â†’ LF line endings  
CI:              regenerates types â†’ compares different line endings â†’ BINARY DIFFER âŒ
```

### After Fix:
```
Local (develop):  types regenerated with CLI v2.34.3 â†’ LF (via gitattributes)
Main branch:      will receive same types after merge
CI:              uses same CLI v2.34.3 â†’ normalizes â†’ identical â†’ PASS âœ…
```
...
**CI (automatic):**
```yaml
npx -y supabase@v2.34.3 gen types typescript --project-id "$SUPABASE_PROJECT_ID" --schema public
```
...
```yaml
run: npx -y supabase@v2.34.3 ...  # Match devDependencies version
```

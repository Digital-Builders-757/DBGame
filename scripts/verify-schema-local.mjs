...
console.log("🧮 Comparing with committed types/supabase.ts...");
const local = readFileSync("types/supabase.ts", "utf8");
...
if (localClean !== remoteTrimmed) {
  console.error("❌ types/supabase.ts is not up-to-date with remote schema.");
  console.error(
    `   Run: npm run types:regen`
  );
  process.exit(1);
}
console.log("✅ types/supabase.ts matches remote schema");
